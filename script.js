...
function startAll() {
  ...
  var vw = $(window).width();
  vh = Math.floor(vh * 0.85);
  vw = Math.floor(vw * 0.85);
  ...
  for (let i = 0; i < rows; i++) {
    ...
    for (let j = 0; j < colms; j++) {
      container.innerHTML +=
        "<div class='unit node" + i + "j" + j + "' ></div>"; //giving the unique class to each node
    }
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < colms; j++) {
      ...
      document
        .querySelector(".node" + i + "j" + j)
        .addEventListener("mousemove", () => addWall(i, j));
      document
        .querySelector(".node" + i + "j" + j)
        .addEventListener("click", () => toggleWall(i, j));
      s = wi - 1 + "px";
      // console.log(s)
      $(".node" + i + "j" + j).width(s);
...
function isValid(i, j) {
  if (i >= 0 && i < rows && j >= 0 && j < colms) {
    if (
      visited.indexOf(".node" + i + "j" + j) == -1 &&
      !document.querySelector(".node" + i + "j" + j).classList.contains("wall")
    ) {
      return true;
    } else return false;
  } else return false;
...
function fillPath_A() {
  ...
  document
    .querySelector(".node" + path[b].i + "j" + path[b].j)
    .classList.add("path");
  b++;
}
